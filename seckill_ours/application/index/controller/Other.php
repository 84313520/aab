...
//        var_dump($arr[0]['city']);exit();
        foreach ($arr as $province) {
            //插入省province.name
            $dataP = Db::name('province')
                ->insert(['prname'=>$province['name']]);
            //获取maxid
            $prid = Db::name('province')->max('prid');
            foreach ($province['city'] as $city) {
                //插入市city[name]
                $dataC = Db::name('city')
                    ->insert(['cityname'=>$city['name'],'prid'=>$prid]);
                //获取市的id
                $cityid = Db::name('city')->max('cityid');
                foreach ($city['area'] as $area) {
                    //插入地区
                    $dataA = Db::name('Area')
                        ->insert(['areaname'=>$area,'cityid'=>$cityid]);
                }
            }
        }
        echo 'the_end';
